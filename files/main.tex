...
\end{figure}
\\\\
Discutindo essa aproximação de $\mathcal{R}_0$ a 1 com o orientador do Trabalho, foi decidido 
que ao invés de aumentar o valor $D_1$, um parâmetro empírico usado para estabilizar a taxa de picadas, seria ideal modificar $b_1$ e $b_2$, que são as proporções de picadas gerando infecção em mosquitos e humanos suscetíveis, respectivamente, tendo em vista que, conforme temos mais ocorrências de áreas desmatadas, haverá um maior contato entre humanos e mosquitos, aumentando a proporção de picadas gerando infecção. Junto com essa correção, também foram modelados os gráficos de evolução das taxas utilizadas, em função da temperatura e precipitação, ao invés do tempo, para que fosse possível analisar o comportamento dessas taxas conforme a temperatura e precipitação variam. Os gráficos estão indicados no apêndice, partindo do 15. Gerando esses gráficos, também foi percebido que seria ideal aumentar $R_L$ do valor atual de 312 mm para 450 mm, para evitar que a probabilidade de sobrevivência de mosquitos durante as diferentes fases se tornasse muito próximo de 0, o que estava afetando a taxa de nascimentos $b(R,T)$.
\\\\
...
\end{cases}
\end{gather*}
A elaboração dos gráficos, a partir do Apêndice 27, foi feita em
\\
https://github.com/RaphaLevy/TCC/blob/main/Modelagem\_com\_Dinamica\_Pop/ \\
...
\begin{figure}[!ht]
        ...
        \caption{SIR com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=1, I_{H0}=1$} 
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.5em} \includegraphics[scale=0.7] {SEI_Entrada_Pop_1_1_Infect.png}}
        \caption{SEI com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=1, I_{H0}=1$} 
\end{figure} 
...
nenhuma diferença visível na evolução da doença, mas com um aumento de 100\%, o resultado ficou como a seguir:
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{3.5em} \includegraphics[scale=0.6] {SIR_Desmat_k=2.png}}
        \caption{SIR com $k=2$}
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{3.0em} \includegraphics[scale=0.6] {SEI_Desmat_k=2.png}}
        \caption{SEI com $k=2$}
\end{figure} 
\newpage
Dobrando a proporção de picadas causando infecção, já é possível notar que o 
número de infectados começa a aumentar, ainda que quase no final do período 
de análise. Isso porque com o valor de $k$, o $\mathcal{R}_0$ passou de 0.83, 
com $k=1$, para 1.66 permitindo que a doença se estabeleça a longo prazo. 
Aumentando $k$ para 5, será bem mais perceptível a evolução da doença:
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{3.7em} \includegraphics[scale=0.6] {SIR_Desmat_k=5.png}}
        \caption{SIR com $k=5$}
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{3.2em} \includegraphics[scale=0.6] {SEI_Desmat_k=5.png}}
        \caption{SEI com $k=5$}
\end{figure} 
\newpage
Nesse caso, $\mathcal{R}_0 = 4.17$, e apesar de não ser possível verificar no tempo 
máximo dos 5 anos, nesse caso o número de infectados já se estabelece positivamente, 
oscilando em aproximadamente 200 humanos e 17000 mosquitos infectados. Aumentando $k$ para 10, 
$\mathcal{R}_0 = 8.34$, e a infecção atinge seu máximo de forma ainda mais rápida:
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{4.2em} \includegraphics[scale=0.6] {SIR_Desmat_k=10.png}}
        \caption{SIR com $k=10$}
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{4.2em} \includegraphics[scale=0.6] {SEI_Desmat_k=10.png}}
        \caption{SEI com $k=10$}
\end{figure} 
...
SIR/SEI para a malária de forma a complementar a metodologia original referenciada, 
...
ambientais, e a necessidade constante de modificar o modelo proposto, tanto 
para corrigir estimações não-realistas, como o modelo oscilante apresentado 
nos primeiros gráficos do trabalho, quanto para alterar a metodologia usada 
no artigo de referência, em especial na modelagem dos mosquitos, a análise 
...
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.3em} \includegraphics[scale=1.0] {Plot_tau_L(T).png}}
...
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.5em} \includegraphics[scale=1.0] {Plot_p_LT(T).png}}
...
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.7em} \includegraphics[scale=1.2] {Plot_p_LRT(R,T).png}}
        \caption*{Apêndice 25: Probabilidade de sobrevivência das larvas $(\tau_{M}(T))$} 
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.7em} \includegraphics[scale=1.2] {Plot_b(R,T).png}}
...
\begin{figure}[!ht]
        ...
        \caption*{Apêndice 27: SIR com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=10000, I_{H0}=500$} 
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.5em} \includegraphics[scale=0.7] {SEI_Entrada_Pop_10000_500_Infect.png}}
        \caption*{Apêndice 28: SEI com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=10000, I_{H0}=500$} 
\end{figure} 
\newpage
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.7em} \includegraphics[scale=0.7] {SIR_Entrada_Pop_100000_1000_Infect.png}}
        \caption*{Apêndice 29: SIR com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=100000, I_{H0}=1000$} 
\end{figure} 
\begin{figure}[!ht]
        \centering
        \hbox{\hspace{2.5em} \includegraphics[scale=0.7] {SEI_Entrada_Pop_100000_1000_Infect.png}}
        \caption*{Apêndice 30: SEI com $T'=25.6, \ A=12.5, \ B=15, \ C=-48.78, \ R_L=450, E_{M0}=100000, I_{H0}=1000$} 
\end{figure} 
